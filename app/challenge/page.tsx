...
function FlashCards({ batch, batchIdx, totalBatches, selectedSet, onExit, onDone }: {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-violet-600 to-indigo-700 flex flex-col">
      ...
      {/* 하단 전체 진행바 */}
      <GradeProgressBar batchIdx={batchIdx} totalBatches={totalBatches} dark />
...
function GradeProgressBar({ batchIdx, totalBatches, dark }: { batchIdx: number; totalBatches: number; dark?: boolean }) {
  const pct = totalBatches > 0 ? Math.round((batchIdx / totalBatches) * 100) : 0;
  return (
    <div className="px-5 pb-5 pt-2">
      <div className="max-w-sm mx-auto">
        <div className="flex justify-between items-center mb-1.5">
          <span className={`text-xs font-bold ${dark ? "text-white/60" : "text-gray-400"}`}>전체 진행도</span>
          <span className={`text-xs font-extrabold ${dark ? "text-white" : "text-violet-600"}`}>{batchIdx}/{totalBatches} 그룹 ({pct}%)</span>
        </div>
        <div className={`h-3 rounded-full ${dark ? "bg-white/20" : "bg-gray-200"}`}>
          <div className={`h-full rounded-full transition-all duration-500 ${dark ? "bg-white" : "bg-violet-500"}`}
            style={{ width: `${pct}%` }} />
        </div>
      </div>
    </div>
